...
A TaskNet specification has three main parts:

### 1. Timelines (State Variables)
...
Timelines model resources and state variables that change over time.
...
task charge {
  ...
  impacts {
    pre { /* effects at start */ }
    maint { /* effects during execution */ }
    post { /* effects at end */ }
  }
}
```

Tasks have durations, constraints, and effects on timelines.

### 3. Constraints (Temporal Properties)
...
Properties that must hold throughout the schedule.
