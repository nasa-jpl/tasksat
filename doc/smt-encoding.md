...
Each task $t \in T$ represents an activity to be scheduled within the horizon. A task is characterized by:
...
We partition tasks as $T = T_{\mathtt{req}} \cup T_{\mathtt{opt}}$ where $T_{\mathtt{req}}$ are required tasks and $T_{\mathtt{opt}}$ are optional tasks.

### 2.2 Timelines

Timelines represent **state variables** that evolve over time as tasks execute. Each timeline $\ell \in L$ has a type that determines its value domain and allowed operations.
...
- $t \in T$ is the task performing the impact
- $\ell \in L$ is the timeline being modified
...
Given a TaskNet $N$, the **scheduling problem** is to find:
...
Given a TaskNet with horizon $H \in \mathbb{N}$ and tasks $T = \{t_1, \ldots, t_n\}$, we partition time into **zones** defined by task boundaries.

**Definition (Zone Boundaries):** Let $Z$ be the number of zones, where $Z = 2|T| + 2$. We introduce zone boundary variables $z_0, z_1, \ldots, z_{Z-1}$ where:
...
For each task $t \in T$, let $s_t$ and $e_t$ denote its start and end time variables. We enforce a bijection between internal zone boundaries $\{z_1, \ldots, z_{Z-2}\}$ and task boundaries $\{s_t, e_t \mid t \in T\}$:

$$\bigwedge_{t \in T} \left( \bigvee_{j=1}^{Z-2} s_t = z_j \right) \land \left( \bigvee_{j=1}^{Z-2} e_t = z_j \right)$$

$$\bigwedge_{j=1}^{Z-2} \bigvee_{t \in T} (s_t = z_j \lor e_t = z_j)$$

**Distinctness:** All task boundaries are pairwise distinct:

$$\bigwedge_{t_i, t_j \in T, i \neq j} (s_{t_i} \neq s_{t_j}) \land (e_{t_i} \neq e_{t_j}) \land (s_{t_i} \neq e_{t_j}) \land (e_{t_i} \neq s_{t_j})$$
...
where $\mathtt{eval}(C, j)$ evaluates condition set $C$ at zone $j$ as:

$$\mathtt{eval}(C, j) = \bigwedge_{\ell \in C} \left( \bigvee_{c \in C[\ell]} \mathtt{test}(\ell, c, j) \right)$$

Here, $C$ is a set of **timeline conditions** (TlCon), where each condition specifies requirements for a timeline $\ell$. For each timeline $\ell$, we have a list of alternatives $C[\ell]$ (a disjunction), and $\mathtt{test}(\ell, c, j)$ checks if timeline $\ell$ satisfies constraint $c$ at zone $j$ (e.g., $\nu^{\ell}[j] \in [a, b]$ or $\sigma^{\ell}[j] = v$). All timelines must satisfy their respective conditions (conjunction over timelines).
...
$$R^{\ell}[i] = \sum_{(t,\ell,w,\sim,r) \in I_R} \mathtt{rate\_active}(t, w, i) \cdot r \cdot \Delta t_i$$
...
$$\nu^{\ell}_{\mathtt{clamped}}[i+1] = \operatorname{max}(b_{\text{min}}, \operatorname{min}(b_{\text{max}}, \nu^{\ell}_{\mathtt{raw}}[i+1]))$$

$$\nu^{\ell}_{\mathtt{clamped}}[i+1] = \operatorname{clamp}(\nu^{\ell}_{\mathtt{raw}}[i+1], b_{\text{min}}, b_{\text{max}})$$
...
For each optional task $t \in T_{\mathtt{opt}}$, introduce a Boolean variable $\iota_t$ indicating inclusion. All constraints for task $t$ are guarded by $\iota_t$:
...
$$\mathtt{minimize} \sum_{t \in T_{\mathtt{opt}}} [\iota_t]$$

**Secondary Objective:** Minimize priority-weighted cost:

$$\mathtt{minimize} \sum_{t \in T} \mathtt{priority}(t) \cdot [\iota_t \lor t \in T_{\mathtt{req}}]$$

**Tertiary Objective:** Minimize deviation from preferred start times:

$$\mathtt{minimize} \sum_{t \in T : \mathtt{start}_t^{\mathtt{pref}} \text{ specified}} |s_t - \mathtt{start}_t^{\mathtt{pref}}|$$

**Quaternary Objective:** Minimize deviation from preferred durations:

$$\mathtt{minimize} \sum_{t \in T : \mathtt{dur}_t^{\mathtt{pref}} \text{ specified}} |(e_t - s_t) - \mathtt{dur}_t^{\mathtt{pref}}|$$

## 9. Soundness and Completeness

**Theorem (Soundness):** If the SMT encoding produces a satisfying model $M$, then the extracted schedule and timeline trace satisfy all TaskNet constraints.
